...
origImg = cv2.imread('img.jpg')
wood = cv2.imread('im27.jpg')
originalImg = img
...
# FLOODFILL
cv2.floodFill(img, resized_image, (1000, 3000), floodfill_color, loDiff=(1, 1, 1, 1), upDiff=((1, 1, 1, 1)))
...
result = cv2.addWeighted(img, 0.9, vChannel, 0.5, 0.0)
# Step 9: Merge with original image
newResult = cv2.addWeighted(img, 0.3, result, 0.7, 0.0)
cv2.imwrite("test/im27.jpg",result)

#.imshow("test", newResult)
...
texture = cv2.imread('yourwoodimagehere.png')
...
cv2.imwrite("test/asd123.jpg",bitwise)


finalResul = cv2.addWeighted(bitwise, 0.7, originalImg, 1, 0.0)
finalResult = cv2.addWeighted(bitwise, 0.00, finalResul, 0.9, 0.0)
...
cv2.imwrite("test/asd4.jpg",finalResult)
